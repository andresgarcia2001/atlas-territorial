...
def test_map_data_accepts_electoral_circuit_level(monkeypatch):
    captured_args = {}
    ...
    response = client.get(
        "/map-data",
        params={"indicator": "poblacion_total", "year": "2022", "level": "electoral_circuit"},
    )

    assert response.status_code == 200
    assert captured_args == {"indicator": "poblacion_total", "year": 2022, "level": "electoral_circuit"}
